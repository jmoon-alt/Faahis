...
const SYSTEM_PROMPT = `أنت محلل عقود قانوني خبير. مهمتك تحليل العقود باللغة العربية وتقديم تقرير واضح ومبسط. قدم النتيجة بصيغة JSON فقط.`;
...
if (level === "مهم جداً") return "#00C48C";
...
setResult(null);
try {
// NOTES FOR CORS: Making a direct client-side call to api.anthropic.com will FAIL.
// To make this work, you need a server-side proxy.
// We'll mock the response to show you how the UI will look.

/*
// This is how it would look with a proxy/server:
...
messages: [{ role: "user", content: `حلل هذا العقد:\n\n${contractText}` }]
})
});
...
// --- MOCK RESPONSE FOR C.O.R.S. BYPASS ---
await new Promise(resolve => setTimeout(resolve, 1500)); // Simulate API delay

// Example of a perfect JSON response from a mock system.
const mockAIResponse = {
"نوع_العقد": "عقد إيجار سكني",
"ملخص": "عقد إيجار لشقة سكنية لمدة سنة واحدة. ينص على التزامات المستأجر والمؤجر.",
"بنود_ضدك": [
{"البند": "إخلاء فوري", "الخطورة": "عالية", "السبب": "يحق للمؤجر إخلاء الشقة فوراً دون سابق إنذار."},
{"البند": "تحمل تكاليف الصيانة", "الخطورة": "متوسطة", "السبب": "يتحمل المستأجر تكاليف الصيانة الدورية التي عادة ما تكون على المؤجر."}
],
"حقوقك": [
{"الحق": "الحق في الخصوصية", "أهميته": "مهم جداً"},
{"الحق": "الحق في استخدام المرافق", "أهميته": "مهم"}
],
"توصية_عامة": "العقد يحتوي على بنود خطرة ويحتاج لتعديل جذري قبل التوقيع."
};
setResult(mockAIResponse);
// ----------------------------------------

} catch (err) {
setError("حدث خطأ أثناء التحليل، حاول مرة ثانية (تأكد من إعداد سيرفر CORS)");
}
setLoading(false);
};
...
⚠️ بنود تضرك ({result["بنود_ضدك"].length})
</div>
...
⚠️ هذا التحليل للاسترشاد فقط وليس استشارة قانونية معتمدة
</p>
